            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-10 ">
                {
                    ...
                    <figure><img className='mx-auto rounded-t-2xl h-fit w-full' src={cls?.image} alt="car!"/></figure>
                    </div>
                    <div className="text-center">
                        <h2 className="font-bold pt-5 text-2xl uppercase">{cls?.course}</h2> <hr className="py-2 mt-2" />
                        <p className='text-gray-500'>Instructor: {cls?.instructorName}</p>
                        <p className=' text-gray-500p'>Available Seats: {cls?.seats}</p>
                        <p className='text-2xl border font-bold w-[40%] mx-auto rounded-lg py-2 my-2'>$ {cls?.price}</p>

                        <button onClick={() => handleSelectClass(cls)} className={('bg-gray-800 px-8 py-2 my-3 text-white hover:bg-slate-900 rounded-lg')}>Select</button>
